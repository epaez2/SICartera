<%@ page import="proyecto.ValorParametro" %>
...
<div class="fieldcontain ${hasErrors(bean: valorParametroInstance, field: 'parametros', 'error')} required">
	...
	<g:select id="parametros" name="parametros.id" from="${proyecto.Parametro.list()}" optionKey="id" required="" value="${valorParametroInstance?.parametros?.id}" class="many-to-one"/>
</div>
